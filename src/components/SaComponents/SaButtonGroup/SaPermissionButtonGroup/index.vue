<template>
  ...
  <div v-if="usePermission" class="sa-permission-button-group">
    ...
    <template v-for="(btn, key) in btnList">
      <template v-if="showBtn(btn)">
        ...
        <el-tooltip
          :key="`permission-tooltip-${btn.key}-${key}`" v-bind="tooltipBindProcess(btn.tooltipBind)"
          :disabled="tooltipBindProcess(btn.tooltipBind).disabled"
        >
          <!--確認視窗-->
          <sa-popover-dialog
            v-if="btn.popover" :key="`permission-pop-${btn.key}-${key}`"
          ...
          <!--上传组件 Start-->
          <sa-upload
            v-else-if="btn.upload" :key="`permission-upload-${btn.key}-${key}`"
          ...
          <el-button :key="`permission-main-${btn.key}-${key}`" v-permission="btn.key" v-bind="buttonBindProcess(btn.bind)" @click="handleButtonClick(btn.key || '')">
            {{ btn.name }}
          </el-button>
        </el-tooltip>
      </template>
  ...
  <div v-else class="sa-permission-button-group">
    ...
    <template v-for="(btn, key) in btnList">
      <template v-if="showBtn(btn)">
        <el-tooltip
          :key="`tooltip-${btn.key}-${key}`" v-bind="tooltipBindProcess(btn.tooltipBind)"
          :disabled="tooltipBindProcess(btn.tooltipBind).disabled"
        >
          <!--確認視窗-->
          <sa-popover-dialog
            v-if="btn.popover" :key="`pop-${btn.key}-${key}`"
            v-bind="popoverBindProcess(btn.popBind)"
            :disabled="buttonBindProcess(btn.bind).disabled"
            @confirm="handleButtonClick(btn.key || '', $event)"
          >
          ...
          <!--上传组件 Start-->
          <sa-upload
            v-else-if="btn.upload" :key="`upload-${btn.key}-${key}`"
          ...
          <el-button v-else :key="`main-${btn.key}-${key}`" v-bind="buttonBindProcess(btn.bind)" @click="handleButtonClick(btn.key || '')">
            {{ btn.name }}
          </el-button>
        </el-tooltip>
      </template>
